...
async function ensureSession(): Promise<boolean> {
  ...
  try {
    ...
    replaceSession(next);
    if (next.isOperator && !isAdminRoute.value) navigate("/admin/overview");
...
async function handleRoute() {
  ...
  if (isOperator.value && !isAdminRoute.value) { navigate("/admin/overview"); return; }
...
async function submitLogin() {
  ...
  try {
    ...
    const allowedRedirect = next.isOperator ? requested?.startsWith("/admin") : requested?.startsWith("/console");
...
<template>
  ...
  <div v-else class="app-shell">
    ...
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      <div class="sidebar-head"><a :href="isOperator ? '/admin/overview' : '/console/overview'" class="brand" @click.prevent="navigate(isOperator ? '/admin/overview' : '/console/overview')"><img src="/opl-app-icon.png" alt="" /><strong>OPL Console</strong></a><button class="sidebar-close" type="button" aria-label="关闭导航" @click="sidebarOpen = false"><X /></button></div>
