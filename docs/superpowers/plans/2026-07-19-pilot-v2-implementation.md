...
(
  ...
  run_go_no_skip env PGHOST=127.0.0.1 PGPORT=5432 PGUSER=postgres PGDATABASE=postgres PGSSLMODE=disable \
    OPL_POSTGRES_TESTS=1 go test -race ./... -count=1
)
(
  cd services/control-plane
  run_go_no_skip env PGHOST=127.0.0.1 PGPORT=5432 PGUSER=postgres PGDATABASE=postgres PGSSLMODE=disable \
    OPL_POSTGRES_TESTS=1 OPL_CAPACITY_TESTS=1 go test ./... -count=1
)
(
  cd services/fabric
  run_go_no_skip env PGHOST=127.0.0.1 PGPORT=5432 PGUSER=postgres PGDATABASE=postgres PGSSLMODE=disable \
    OPL_POSTGRES_TESTS=1 go test ./... -count=1
)
(
  cd services/ledger
  run_go_no_skip env PGHOST=127.0.0.1 PGPORT=5432 PGUSER=postgres PGDATABASE=postgres PGSSLMODE=disable \
