...
> **状态：待 owner 批准，不得执行。** 本文只定义实施顺序、原子提交和验收门禁；
> 当前计划提交不得包含功能代码、部署或外部写操作。

> **实施纪律：** 每个切片在独立 clean worktree 中完成：先实际运行新增测试取得 RED，
> 再做最小实现，focused PASS 后运行该切片的 module/full PASS，形成一个原子 commit；
> integration owner 只 cherry-pick 通过的 commit。故意失败的测试状态不提交到 integration 分支。

**目标：** 以 owner 批准的冻结稿 `73c0ec09f3eff301cb4df842fda804c26ed37120`
...
| Pilot V2 冻结稿 | `73c0ec09f3eff301cb4df842fda804c26ed37120` | 唯一设计输入 |
...
以下是 hard cut 后唯一 Console 产品路由。JSON 使用 `ProductSourceEnvelope<T>`：
`source` 只允许 `controlPlane|gateway|fabric|runtime|ledger`，`status` 只允许
`available|empty|unavailable`，并携带权威 `sourceUpdatedAt`；失败响应不能伪造零值或空集合。
...
`ProductSourceEnvelope<T>`、`MoneyDTO` 和 `OperationStatusDTO`。不引入 DTO 生成器或新依赖。
...
子资源扣款，且没有 Runtime 文件、钱包调整和公告 owner。

**最小实现：** 只改合同、说明和命名 DTO；`deliveryEvidence` 初始仍为 `codeComplete=false`，
...
**写闭环：** owner + CSRF -> 名称/额度/到期/Key 所有权校验 -> `Idempotency-Key` 派生 operation ->
...
**API / 命名 DTO：** 实现第 3 节 Key、Key secret、Key Usage、账号 Usage 路由。
`GatewayKeySummaryDTO.kind` 为 `general|workspace`；`manageable` 和 `deletable` 由服务端给出。
`CreateGatewayKeyRequest` 只允许 `name`、`quotaUsdMicros`、`expiresAt`；拒绝 custom raw Key、
group、user/account ID 和 `opl-workspace`/内部轮换前缀。Usage 服务端固定 remote user ID + path Key ID。
...
```bash
cd services/control-plane
go test ./internal/clients -run 'TestUser(KeyCreateIdempotent|KeyUpdate|KeyDelete|KeyUsage)' -count=1
go test ./internal/server -run 'TestGateway(GeneralKey|KeyOwnership|KeySecret|PerKeyUsage|AccountUsageSummary)' -count=1
```
...
RuntimeOperation 和 source envelope；不创建 Gateway service、不引入 repository abstraction。

**focused PASS：**

```bash
cd services/control-plane
go test ./internal/clients ./internal/server -run 'Test(UserKey|GatewayGeneralKey|GatewayKeyOwnership|GatewayKeySecret|GatewayPerKeyUsage|GatewayAccountUsageSummary)' -count=1
```
...
- Add: `services/control-plane/internal/server/workspace_key_convergence_test.go`
...
go test ./internal/server -run 'TestWorkspaceKey(Convergence|Rotation|Secret|Ambiguity|NoRawPersistence)' -count=1
```

预期 RED：Workspace schema 无 Key ID，启动要求预存一个 Key，现有 gateway-secret rotate 只重写
同一 Key，不能完成 Key 生命周期轮换。
...
```bash
cd services/control-plane
go test ./migrations ./internal/clients ./internal/server -run 'Test(WorkspaceAPIKeyIDMigration|WorkspaceKey)' -count=1
```
...
显示真实更新时间/缓存新鲜度。一个远端账号异常只标记对应来源，不把所有指标补零。
...
`OperatorWorkspacePageDTO`、`OperatorReconciliationPageDTO`、`OperatorHealthDTO`。批量上限
...
go test ./internal/server -run 'TestOperator(ProjectionUsesBatchAPIs|ProjectionPartialFailure|ProjectionHasNoNPlusOne|AccountInvite|AccountDisable)' -count=1
```
...
公告；不需要理解内部服务，也不会因一个来源失败失去其他已确认事实。
...
超时、logout 立即清空。自动续费在真实 Basic/Pro renewal evidence 前保持 disabled 并显示状态原因。
...
**API / 命名 DTO：** 使用 S1 `dtos.ts` 全部用户 DTO。Base URL 只来自服务端 OPL branded config；
缺配置显示 unavailable，禁止 gflabtoken link/iframe/fallback。新增 exact `formatUsd` 复用现有整数
formatter，不用浮点推导金额。
...
```bash
node --test --test-name-pattern='Pilot V2 customer' tests/ui/*.test.ts
npm run typecheck
```
...
依赖，不先做无关组件重构。页面按真实任务组织，敏感 reveal 使用现有 modal/panel pattern。

**focused PASS：**

```bash
node --test tests/ui/pilot-v2-customer-flow.test.ts tests/ui/gateway-request-lifecycle.test.ts tests/ui/balance-availability.test.ts
npm run typecheck
```
...
**运维表现：** Overview 指标明确口径；Accounts 分页展示映射/余额/Key/Workspace；Resources 展开真实
provider ID/规格/Zone/到期/读回时间；Billing 展示 balance history、Workspace Receipt、Fabric op 和
reconciliation 异常；Health 只展示真实 probe/时间；Announcements 完整状态机。Accounts 提供邀请和
禁用，不提供删除。所有危险命令二次确认。

**Owner / 真相 / 写入方：** UI 不拥有任何事实，只消费 S5/S6/S8-S10 operator DTO；不能显示 raw Key、
密码、上游响应、数据库字段或管理员 token。
...
`OperatorHealthDTO`、`OperatorAnnouncementPageDTO`。健康 DTO 没有采样时为 unavailable，不根据
空错误列表推断 healthy。
...
```bash
node --test --test-name-pattern='Pilot V2 operator' tests/ui/*.test.ts
npm run typecheck
```

预期 RED：当前 operator 页面没有批量总览、钱包 adjustment dialog、文件/真实用量、完整 health 和公告。
...
### Slice 14：PostgreSQL、浏览器、结构检查、全量与真实环境分级门禁

**业务结果：** 同一 integration HEAD 对数据库并发/迁移、桌面/移动交互、owner 边界、全量测试和
真实环境证据有可重复门禁；任何层级都不能用较低等级证据冒充较高等级结论。

**用户/运维表现：** 本地 Playwright harness 覆盖用户/运维所有 available/empty/unavailable/error、
重复提交、secret 清理和响应式布局；它拦截 API 且禁止访问真实 Sub2API、腾讯、模型或生产环境。
...
```bash
node --test tests/ui/pilot-v2-browser-harness.test.ts tests/production/production-verifier.test.ts tests/production/production-live-qa.test.ts
```

预期 RED：当前没有 V2 fake-only desktop/mobile harness，也没有 V2 evidence level/allow-flag 断言。
...
env PGHOST=127.0.0.1 PGPORT=5432 PGUSER=postgres PGDATABASE=postgres PGSSLMODE=disable OPL_POSTGRES_TESTS=1 go test -race ./... -count=1

cd ../../control-plane
env PGHOST=127.0.0.1 PGPORT=5432 PGUSER=postgres PGDATABASE=postgres PGSSLMODE=disable OPL_POSTGRES_TESTS=1 go test ./... -count=1
...
要求：Node SKIP 0；Go JSON test log 无 `Action=skip`；PostgreSQL migrations/CAS、desktop 1440x900、
mobile 390x844、用户/运维两角色、available/empty/unavailable/error/secret cleanup 全部通过；Sentrux
...
本计划 commit 后暂停。没有 owner 对本文件的明确批准，不创建 S1-S14 功能 worktree，不运行 RED，
不修改 Sub2API/Runtime，不执行 migration、部署、真实收费、腾讯资源或模型请求，也不 push/PR/merge。
