...
清退边界和验收门槛。本文获 owner 批准前只是候选设计，不改变
`docs/invariants.md` 和现有 JSON 机器合同的 `current` 状态，也不证明任何能力已经部署。
...
URL、Runtime 状态，以及计算/存储/挂载的当前状态。展开资源可查看供应商资源 ID、规格、Zone、
开通时间、到期时间、最近读回时间和失败原因。
