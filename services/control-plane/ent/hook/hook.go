...
// The SupportTicketMappingFunc type is an adapter to allow the use of ordinary
// function as SupportTicketMapping mutator.
type SupportTicketMappingFunc func(context.Context, *ent.SupportTicketMappingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SupportTicketMappingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SupportTicketMappingMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SupportTicketMappingMutation", m)
}
