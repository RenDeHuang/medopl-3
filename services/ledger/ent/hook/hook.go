...
import (
	"context"
	"fmt"
	"opl-cloud/services/ledger/ent"
)

// The EvidenceReceiptFunc type is an adapter to allow the use of ordinary
// function as EvidenceReceipt mutator.
type EvidenceReceiptFunc func(context.Context, *ent.EvidenceReceiptMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f EvidenceReceiptFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.EvidenceReceiptMutation); ok {
		return f(ctx, mv)
	}
...
// function as ReconciliationReport mutator.
type ReconciliationReportFunc func(context.Context, *ent.ReconciliationReportMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ReconciliationReportFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ReconciliationReportMutation); ok {
		return f(ctx, mv)
	}
